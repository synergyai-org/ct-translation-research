...
def train_syndiff(rank, gpu, args):
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x1, x2) in enumerate(data_loader):
            logger.info(f"Epoch {epoch}, iteration {iteration}")
            logger.info(x1.shape)
            ...
            logger.info("input real data to D1, D2")                        
            ...
            logger.info(real_data1.shape)
            logger.info(real_data2.shape)

            x1_0_predict = gen_non_diffusive_2to1(real_data2)
            x2_0_predict = gen_non_diffusive_1to2(real_data1)       

            logger.info(x1_0_predict.shape)
            logger.info(x2_0_predict.shape) 

            # Given groups=1, weight of size [64, 1, 3, 3], expected input[3, 2, 512, 512] to have 1 channels, but got 2 channels instead
